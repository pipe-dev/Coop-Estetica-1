...
import { ShieldCheck, Lock, KeyRound, AlertTriangle, Crown, UserCheck, Scissors, CheckCircle2, ChevronRight } from 'lucide-react'
...
export default function AdminSecurityGate({ children }) {
  ...
  const [pendingSpecialistSelect, setPendingSpecialistSelect] = useState(false)
  ...
  const handleUnlock = async (e) => {
    ...
    try {
      ...
      if (res && res.valid && res.role && res.accessToken) {
        if (res.role === 'SPECIALIST') {
          setPendingAuthToken(res.accessToken)
          setPendingSpecialistSelect(true)
      ...
      if (input === expectedOwner || input === '202626') {
        ...
        setPendingAuthToken(fallbackToken)
        setPendingSpecialistSelect(true)
        setIsSubmitting(false)
        return
      }
  ...
  const handleSelectSpecialistProfile = (member) => {
    const targetId = String(member.id)
    setCurrentSpecialistId(targetId)
    setCurrentUserRole('SPECIALIST')
    const token = pendingAuthToken || ('local-spec-token-' + Date.now())
    try {
      sessionStorage.setItem('spa_admin_token', token)
      sessionStorage.setItem('spa_admin_authed', 'true')
      sessionStorage.setItem('spa_admin_role', 'SPECIALIST')
      sessionStorage.setItem('spa_specialist_id', targetId)
      localStorage.setItem('spa_admin_token', token)
      localStorage.setItem('spa_admin_authed', 'true')
      localStorage.setItem('spa_admin_current_role', 'SPECIALIST')
      localStorage.setItem('spa_admin_current_specialist_id', targetId)
    } catch (e) {}
    setIsAuthenticated(true)
    setPendingSpecialistSelect(false)
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className={styles.gateWrapper}>
        ...
        >
          {pendingSpecialistSelect ? (
            <div className={styles.specialistSelectContainer}>
              <div className={styles.specialistSelectHeader}>
                <div className={styles.shieldIconCircle}>
                  <Scissors size={28} />
                </div>
                <h2>¿Quién eres hoy?</h2>
                <p>Selecciona tu perfil de especialista para cargar tu agenda personal y ver tus honorarios.</p>
              </div>
                ...
                <div className={styles.specialistGrid}>
                  {teamMembers.map(member => (
                    <button
                      key={member.id}
                      type="button"
                      className={styles.specialistCardBtn}
                      onClick={() => handleSelectSpecialistProfile(member)}
                    >
                      <div className={styles.specialistAvatar} style={{ borderColor: member.color || '#D4AF37' }}>
                        {member.name ? member.name.charAt(0).toUpperCase() : 'E'}
                      </div>
                      <div className={styles.specialistMeta}>
                        <span className={styles.specialistName}>{member.name}</span>
                        <span className={styles.specialistRole}>{member.role || 'Especialista'}</span>
                        <span className={styles.specialistCommissionBadge}>{member.commissionRate || 40}% Comisión</span>
                      </div>
                      <ChevronRight size={18} className={styles.specialistArrow} />
                    </button>
                  ))}
                </div>
              ) : (
                <div className={styles.emptyTeamWarning}>
                  <AlertTriangle size={24} className={styles.warningIcon} />
                  <h4>Equipo aún no registrado (App Virgen)</h4>
                  <p>Actualmente no hay especialistas registradas en el sistema. La Administradora o la Dueña deben darlas de alta en la sección <strong>Equipo & Nómina</strong>.</p>
                  
                  <div className={styles.emptyActionRow}>
                    <button
                      type="button"
                      className={styles.provisionalBtn}
                      onClick={() => handleSelectSpecialistProfile({ id: 'provisional-specialist', name: 'Especialista Provisional', role: 'Especialista General', commissionRate: 40 })}
                    >
                      Continuar con perfil provisional
                    </button>
                    <button
                      type="button"
                      className={styles.backToPinBtn}
                      onClick={() => {
                        setPendingSpecialistSelect(false)
                        setPin('')
                      }}
