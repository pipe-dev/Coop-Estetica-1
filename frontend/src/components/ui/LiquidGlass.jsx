import React, { useId } from 'react'

// The ultra-optimized WebP normal map for realistic glass edge refraction/magnification
const WEBP_DISPLACEMENT_MAP = "data:image/webp;base64, BRPUgOo6nklsaa3T8bJR8sC1Bh4OJ9is/aAD";

export const LiquidGlass = ({ children, borderRadius = 48, className = '' }) => {
  const filterId = useId().replace(/:/g, "");
  
  const currentPath = typeof window !== 'undefined' ? window.location.pathname + window.location.search : '';
  const filterUrl = `${currentPath}#liquid-glass-${filterId}`;

  return (
    <div 
      className={className}
      style={{ 
        position: 'relative',
        borderRadius,
        overflow: 'visible',
      }}
    >
      <svg 
        style={{ 
          position: 'absolute', 
          width: '1px', 
          height: '1px', 
          opacity: 0.01, 
          pointerEvents: 'none', 
          overflow: 'hidden',
          zIndex: -100 
        }} 
        aria-hidden="true"
      >
        <filter id={`liquid-glass-${filterId}`} primitiveUnits="objectBoundingBox" x="-20%" y="-20%" width="140%" height="140%">
          {/* 1. Normal Map (WebP) for the magnifying edge glass refraction */}
          <feImage 
            result="edgeMap" 
            width="100%" 
            height="100%" 
            x="0" 
            y="0" 
            href={WEBP_DISPLACEMENT_MAP} 
            preserveAspectRatio="none" 
          />
          {/* 2. High-frequency turbulence to simulate a prism defragmenter/crystal shard texture */}
          <feTurbulence type="fractalNoise" baseFrequency="0.08" numOctaves="3" result="rippleNoise" />
          {/* Normalize and isolate noise color channels */}
          <feColorMatrix type="matrix" values="1 0 0 0 0.5  0 1 0 0 0.5  0 0 1 0 0  0 0 0 0.3 0" in="rippleNoise" result="softRipple" />
          {/* 3. Blend edge displacement with ripple noise */}
          <feBlend mode="overlay" in="edgeMap" in2="softRipple" result="combinedMap" />
          
          {/* 4. Lower displacement scale (0.05) to keep text legible and prevent massive magnification */}
          <feDisplacementMap 
            in="SourceGraphic" 
            in2="combinedMap" 
            scale="0.05" 
            xChannelSelector="R" 
            yChannelSelector="G" 
          />
        </filter>
      </svg>

      {/* DEFENSIVE CSS INJECTION TO PROVIDE CROSS-BROWSER FALLBACKS */}
      <style>{`
        .glass-base-${filterId} {
          position: absolute;
          top: 0; left: 0; right: 0; bottom: 0;
          z-index: 0;
          pointer-events: none;
          background-color: rgba(255, 255, 255, 0.002);
          
          /* Safe fallback */
          backdrop-filter: blur(2px) saturate(100%);
          -webkit-backdrop-filter: blur(2px) saturate(100%);
          
          /* Apply the prism ripple to the whole navbar background */
          backdrop-filter: blur(2px) url(${filterUrl}) saturate(100%);
          -webkit-backdrop-filter: blur(2px) url(${filterUrl}) saturate(100%);
        }

        .glass-bevel-${filterId} {
          position: absolute;
          top: 0; left: 0; right: 0; bottom: 0;
          z-index: 1;
          pointer-events: none;
          -webkit-mask-image: 
            linear-gradient(to bottom, #000 0%, rgba(0,0,0,0) 22px, rgba(0,0,0,0) calc(100% - 22px), #000 100%),
            linear-gradient(to right, #000 0%, rgba(0,0,0,0) 22px, rgba(0,0,0,0) calc(100% - 22px), #000 100%);
          -webkit-mask-composite: source-over;
          mask-composite: add;
          box-shadow: inset 0 1px 2px rgba(255,255,255,0.4), inset 0 -1px 2px rgba(0,0,0,0.3);
          backdrop-filter: blur(16px) saturate(200%) brightness(1.15);
          -webkit-backdrop-filter: blur(16px) saturate(200%) brightness(1.15);
          backdrop-filter: blur(16px) url(${filterUrl}) saturate(200%) brightness(1.15);
          -webkit-backdrop-filter: blur(16px) url(${filterUrl}) saturate(200%) brightness(1.15);
        }

        .glass-lighting-${filterId} {
          position: absolute;
          top: 0; left: 0; right: 0; bottom: 0;
          z-index: 2;
          pointer-events: none;
          box-shadow: 
            inset 0 0 0 1px rgba(255, 255, 255, 0.15),
            inset 0 2px 3px -1px rgba(255, 255, 255, 0.5), 
            inset 2px 0px 3px -1px rgba(255, 255, 255, 0.4), 
            inset 0 -2px 3px -1px rgba(0, 0, 0, 0.4), 
            inset -2px 0px 3px -1px rgba(0, 0, 0, 0.2);
          background-image: linear-gradient(180deg, rgba(255,255,255,0.03) 0%, rgba(0,0,0,0) 40%, rgba(0,0,0,0.04) 100%);
        }

        .glass-content-${filterId} {
          position: relative;
          z-index: 10;
          width: 100%;
          height: 100%;
          padding: 6px;
          filter: url(${filterUrl});
          -webkit-filter: url(${filterUrl});
        }
      `}</style>

      <div 
        className={`glass-base-${filterId}`}
        style={{ borderRadius: 'inherit' }}
      />
      <div 
        className={`glass-bevel-${filterId}`}
        style={{ borderRadius: 'inherit' }}
      />
      <div 
        className={`glass-lighting-${filterId}`}
        style={{ borderRadius: 'inherit' }}
      />
      <div className={`glass-content-${filterId}`}>
        {children}
      </div>
    </div>
  )
}

export default LiquidGlass
